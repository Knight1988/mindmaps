...
mindmaps.DefaultCanvasView = function () {
    ...
    captionEditor.commit = function (node, text, tooltip) {
        if (self.nodeCaptionEditCommitted) {
            self.nodeCaptionEditCommitted(node, text, tooltip);
        }
    };
    ...
    this.init = function () {
        ...
        // setup delegates
        $drawingArea.delegate("div.node-container", "mousedown", function (e) {
            e.stopPropagation();
    ...
    this.createNode = function (node, $parent, depth) {
        ...
        if (!node.isRoot()) {
            ...
            function enableDragging() {
                if (!self.isEditing) {
                    $node.one("mouseenter", enableDragging);
                } else {
                    $node.draggable({
                        // could be set
                        // revert: true,
        ...
        var $text = $("<div/>", {
            ...
            tooltip.show($text, node.getTooltip());
        }).bind("mouseleave.tooltip", function () {
            tooltip.hide();
        }).mousedown(function (e) {
            if ($(this).hasClass("edit")) e.stopPropagation();
        }).appendTo($node);
     ...
     */
    this.setNodeText = function (node, caption) {
        var $text = $getNodeCaption(node);
        var metrics = textMetrics.getTextMetrics(node, this.zoomFactor, caption);

        $text.css(metrics).text(caption);
    };
    ...
    /**
     * Creates a new Tooltip. This tool offers an tooltip component.
     * 
     * @constructor
    ...
    function Tooltip(view) {
        ...
        /**
         * Attaches the tooltip to the node.
         * 
         * @param {jQuery} $text
         * @param {String} text
         */
        this.show = function ($text, text) {
            $tooltip.html(text);
    ...
    function CaptionEditor(view) {
        ...
        var $editor = $("<textarea/>", {
            id: "caption-editor",
            "class": "caption-editor node-text-behaviour"
        ...
        function commitText() {
            if (attached && self.commit) {
                self.commit(self.node, $editor.val(), $tooltip.val());
            }
        }
        ...
        this.edit = function (node, $cancelArea) {
            ...
            this.$text.unbind("mouseenter.tooltip").unbind("mouseleave.tooltip");
            ...
            $tooltip.attr({
                value: this.tooltip
            }).css(metrics).appendTo(this.$text);
        };
        ...
        this.stop = function () {
            if (attached) {
                ...
                // rebind tooltip
                this.$text.bind("mouseenter.tooltip",
                        function () {
                            tooltip.show(self.$text, self.node.getTooltip());
                        })
