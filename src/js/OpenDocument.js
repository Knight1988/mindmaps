...
mindmaps.OpenDocumentView = function() {
    ...
    var $openCloudButton = $("#button-open-cloud")
        .button()
        .click(function() {
    ...
    $dialog.find(".file-chooser input")
        .bind("change",
            function(e) {
    ...
    var $db = $dialog.find(".db-filelist");
    $db.delegate("a.title",
            "click",
            function() {
                if (self.documentClicked) {
        ...
        .delegate("a.delete",
            "click",
            function() {
                if (self.deleteDocumentDbClicked) {
                    var t = $(this).tmplItem();
                    self.deleteDocumentDbClicked(t.data);
                }
            });
    ...
    */
    this.renderDatabase = function(docs) {
        // empty list and insert list of documents
        var $list = $(".document-list-db", $dialog).empty();
    ...
    * @param {mindmaps.Document[]} docs
    */
    this.showOpenDialog = function(docs, dbDocs) {
        this.render(docs);
        if (dbDocs) this.renderDatabase(dbDocs);
        $dialog.dialog("open");
    };
...
mindmaps.OpenDocumentPresenter = function(eventBus, mindmapModel, view, filePicker) {
    ...
    */
    ...
    /**
    * View callback: The delete link the database list has been clicked.
    * Delete the document, and render list again.
    * 
    * @ignore
    * @param {mindmaps.Document} doc
    */
    view.deleteDocumentDbClicked = function(doc) {
        MindMapServiceAPI.remove(doc, function(docs) {
            // re-render view on success
            view.renderDatabase(docs);
        }, function(msg) {
            view.showCloudError(msg);
        });
    };
